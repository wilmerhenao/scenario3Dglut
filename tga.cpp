...
std_fread(RLEstate *rleInfo, unsigned char *buf, size_t datasize, size_t nelems, FILE *fp){
    if (verbose > 1) {
	//count how many bytes have been read from teh file
	totbytes += nelems * datasize;
	printf("TGA: std_fread %zu (total %d)\n",
	       nelems * datasize, totbytes);
    }
    // read a certain number of elements
    return fread(buf, datasize, nelems, fp);
}

// min macro.  Try to replace
...
rle_fread(RLEstate *rleInfo, unsigned char *vbuf, size_t datasize, size_t nelems, FILE *fp){
	...
	if (count & RLE_PACKETSIZE) {
		...
		if (verbose) {
		    printf("TGA: EOF while reading %d/%zu element RLE packet\n",
			   bytes, datasize);
		}
		return j / datasize;
	    }
		...
		if (verbose) {
		    printf("TGA: EOF while reading %d/%zu element raw packet\n",
			   bytes, datasize);
		}
		return j / datasize;
	    }
    ...
    if (verbose > 1) {
	printf("TGA: rle_fread %zu/%d (total %d)\n",
...
gliGenericImage *
gliReadTGA(FILE *fp, const char *name){
    TgaHeader tgaHeader;
    TgaFooter tgaFooter;
